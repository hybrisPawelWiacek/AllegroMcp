...
const POLISH_CARRIERS = ['InPost', 'DPD', 'DHL', 'UPS', 'Poczta Polska', 'GLS'];
...
function generatePolishPostCode() {
...
function generatePolishPersonalData() {
...
function generateMockOrder(orderId) {
    ...
    return {
        id,
        status: randomChoice(['NEW', 'PROCESSING', 'READY_FOR_SHIPMENT', 'SENT', 'DELIVERED']),
        createdAt,
        ...
        payment: {
            id: generateUUID(),
            type: randomChoice(['ONLINE', 'WIRE_TRANSFER', 'CASH_ON_DELIVERY']),
            provider: randomChoice(['PAYU', 'P24', 'ALLEGRO_PAY']),
...
function generateOrderEvent() {
    return {
        id: generateUUID(),
        type: randomChoice(['ORDER_STATUS_CHANGED', 'FULFILLMENT_STATUS_CHANGED', 'PAYMENT_STATUS_CHANGED']),
        order: {
            checkoutForm: {
                id: generateUUID()
            }
...
class MockOrderStore {
    ...
    async listOrders(limit = 20, offset = 0) {
